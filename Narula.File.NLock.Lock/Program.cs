	...
	static void Main(string[] args)
	{
		...
		if (args.Length >= 2 && (args[0] == "-json64" || args[0] == "-json"))
		{
			try
			{
				...
				var launchReq = JsonSerializer.Deserialize<LockGuiLaunchRequest>(json, new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.CamelCase });
				Application.Run(new LockForm(launchReq));
				return;
			}
